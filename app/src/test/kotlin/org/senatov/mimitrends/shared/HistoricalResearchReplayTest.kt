...
class HistoricalResearchReplayTest {
    @Test
    fun `never passes future bars into detector or feature extraction`() {
        val sessionStart = 1_767_615_600L
        val bars = (0 until 6).flatMap { day ->
            (0..120).map { minute ->
                val price = 100.0 + day + minute * 0.01
                MinuteBar(
                    "TEST", sessionStart + day * 86_400L + minute * 60L,
        ...
        samples.forEach { sample ->
            assertEquals(listOf(5, 10, 30, 60, 90), sample.outcomes.map { it.horizonMinutes })
