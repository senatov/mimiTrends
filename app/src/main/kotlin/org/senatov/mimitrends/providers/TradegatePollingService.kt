...
internal class TradegatePollingService(
    ...
    override fun configure(criteria: ScannerCriteria) {
        ...
        intervalMillis = criteria.tradegateRequestIntervalMillis.coerceIn(500, 10_000)
        scheduleNext(0, generation)
        log.info(LogTag.API, "Tradegate provider started symbols={} interval={}ms", symbols.size, intervalMillis)
    }
            ...
            runCatching { poll(symbol) }
                ...
                .onFailure { error ->
                    ...
                    if (error is ProviderHttpException && isPermanentInstrumentStatus(error.statusCode)) {
                        ...
                        backoff.success()
                        log.warn(
                            LogTag.API, "Tradegate instrument disabled symbol={} status={} retryIn={}h",
    ...
    private fun resolve(symbol: String): ProviderInstrument? {
        ...
        repository.loadProviderInstrument(PROVIDER, symbol)?.let { cached ->
            if (isLikelyEquity(cached) &&
                ...
                ProviderInstrumentSelector.matchesCompany(symbol, companyName, cached.resolvedName)
            ) return cached
            repository.deleteProviderInstrument(PROVIDER, symbol)
        }
        ...
        if (!ProviderInstrumentSelector.matchesCompany(symbol, companyName, resolved.name)) {
            unresolvedUntil[symbol] = now + UNRESOLVED_RETRY_MILLIS
            log.warn(
                LogTag.API, "Tradegate instrument rejected symbol={} expectedName={} resolvedName={}",
