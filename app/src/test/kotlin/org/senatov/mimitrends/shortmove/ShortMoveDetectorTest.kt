...
class ShortMoveDetectorTest {
    @Test
    fun `selects holding gains and moderate positive moves`() {
        val selected = ModeratePositiveCandidateSelector.select(
            listOf(
                move("STEADY", 0.55).copy(
                    barCount = 5, safetyScore = 76, safetyConfidence = 80,
                    entryQualityScore = 75
                ),
                move("FLAT", 0.05).copy(
                    barCount = 5, safetyScore = 62, safetyConfidence = 60,
                    entryQualityScore = 68
                ),
                move("JUMP", 2.0).copy(
                    barCount = 5, safetyScore = 90, safetyConfidence = 90,
                    entryQualityScore = 80
                ),
                move("DOWN", -0.4).copy(
                    barCount = 5, safetyScore = 40, safetyConfidence = 80,
                    entryQualityScore = 70
                ),
                move("SPECIAL", 0.6).copy(
                    barCount = 5, pattern = ShortMovePattern.RECURRING_SHARP_JUMP,
    ...
    @Test
    fun `detects a new sharp rise when similar rises recur across days`() {
        val day = 86_400L
        val now = 3 * day + 180L
        val bars = listOf(
    ...
    @Test
    fun `detects a new sharp drop in the same recurring movement pattern`() {
        val day = 86_400L
        val now = 3 * day + 180L
        val bars = listOf(
            bar("NDA.DE", 60L, 100.0, 100.0), bar("NDA.DE", 120L, 104.0, 104.0),
    ...
    fun `keeps the full retained drop after volatile trading near the low`() {
        ...
        val recent = closes.mapIndexed { index, close ->
            bar(
                "LVMH", now - (closes.lastIndex - index) * 60,
    ...
    fun `detects danaher recovery after an extended drop at sixteen fifty eight`() {
        ...
        val bars = prices.mapIndexed { index, close ->
            bar(
                "DHR", now - (prices.lastIndex - index) * 60,
    ...
    fun `detects palantir recovery with one quarter of the drop retained at seventeen forty nine`() {
        ...
        val bars = prices.mapIndexed { index, close ->
            bar(
                "PLTR", now - (prices.lastIndex - index) * 60,
                if (index == 0) close else prices[index - 1], close
            )
        }
