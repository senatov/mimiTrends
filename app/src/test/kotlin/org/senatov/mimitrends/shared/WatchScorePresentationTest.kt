...
class WatchScorePresentationTest {
    @Test
    fun `poor entry quality prevents a buy despite a positive signal`() {
    ...
    @Test
    fun `unconfirmed bottom is never presented as a buy`() {
        val score = WatchScorePresentation.calculate(
    ...
    @Test
    fun `strong rise without volume or outcome confirmation remains wait`() {
        val score = WatchScorePresentation.calculate(
    ...
    @Test
    fun `confirmed strong rise can reach buy category`() {
        val score = WatchScorePresentation.calculate(
            TestScanResult.create(anomalyScore = 4.56, signalSource = "Steady rise ↑").copy(
    ...
    @Test
    fun `strong trend cannot compensate for a late entry`() {
        val score = WatchScorePresentation.calculate(
            TestScanResult.create(anomalyScore = 6.0, signalSource = "Steady rise ↑").copy(
    ...
    @Test
    fun `aging signal cannot remain buy`() {
        val score = WatchScorePresentation.calculate(
            TestScanResult.create(anomalyScore = 6.0, signalSource = "Steady rise ↑").copy(
                relativeVolume = 3.0,
                signalAgeMinutes = 12
    ...
    @Test
    fun `stale signal is always avoid`() {
        val score = WatchScorePresentation.calculate(
            TestScanResult.create(anomalyScore = 6.0, signalSource = "Steady rise ↑").copy(
                relativeVolume = 3.0,
    ...
    @Test
    fun `penalizes an extended entry without discarding a strong instrument`() {
    ...
    @Test
    fun `missing executable quote cannot produce buy`() {
        val score = WatchScorePresentation.calculate(
    ...
    @Test
    fun `spread wider than plausible short move is avoid`() {
        val score = WatchScorePresentation.calculate(
    ...
    @Test
    fun `weak calibrated downside cannot be presented as buy`() {
        val score = WatchScorePresentation.calculate(
            TestScanResult.create(signalSource = "Steady rise ↑").copy(
    ...
    @Test
    fun `negative latest three minutes override an earlier bullish jump`() {
        val score = WatchScorePresentation.calculate(
            TestScanResult.create(signalSource = "Steady rise ↑").copy(
                relativeVolume = 3.0, bidPrice = 100.0, askPrice = 100.04,
    ...
    @Test
    fun `cyclic latest tail cannot remain buy`() {
        val score = WatchScorePresentation.calculate(
            TestScanResult.create(signalSource = "Steady rise ↑").copy(
                relativeVolume = 3.0, bidPrice = 100.0, askPrice = 100.04,
    ...
    @Test
    fun `uses a separate traffic light color scale`() {
