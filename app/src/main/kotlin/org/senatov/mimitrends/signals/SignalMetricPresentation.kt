...
internal object SignalMetricPresentation {
    ...
    fun strength(result: ScanResult): SignalMetric {
        ...
        return SignalMetric(
            "%.0f%%".format(percentile), anomalyColor(percentile), if (score >= 4.0) 600 else 500,
    ...
    fun outcome(result: ScanResult): SignalMetric {
        if (!result.medianNetReturnPercent.isFinite() && result.predictionSource == "LOGISTIC") {
            ...
            return SignalMetric(
                "Model %.0f%%".format(probability), modelColor(probability), 500,
        ...
        if (!result.medianNetReturnPercent.isFinite()) {
            if (result.continuationProbability.isFinite()) {
                val probability = result.continuationProbability * 100.0
                return SignalMetric(
                    "Beta %.0f%%".format(probability), modelColor(probability), 500,
    ...
    fun priceAction(result: ScanResult): SignalMetric {
        ...
        return SignalMetric(
            arrows(result), color, if (jump >= 4.0 || range >= 5.0) 600 else 500,
            "$label\nPrice jump: %.2fσ\nFull candle range: %.2fσ\n10-minute move: %+.2f%%".format(
                jump,
                range,
                result.windowChangePercent
            )
        )
    }
    ...
    fun volume(result: ScanResult): SignalMetric {
        ...
        return SignalMetric(
            label, level.color, if (level in setOf(Level.EXTREME, Level.STRONG)) 600 else 500,
