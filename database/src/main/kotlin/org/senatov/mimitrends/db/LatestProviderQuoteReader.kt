...
internal object LatestProviderQuoteReader {
    ...
    fun load(connection: Connection, provider: String, symbol: String, notBeforeMillis: Long): ProviderQuoteSnapshot? =
        connection.prepareStatement(
            """SELECT provider, identifier, currency, last, bid, ask, bid_size, ask_size, session_volume,
                session_turnover, average_price, executions, session_high, session_low, previous_close, observed_at
                FROM provider_quotes WHERE provider=? AND symbol=? AND observed_at>=?
                ORDER BY observed_at DESC LIMIT 1"""
        ).use { statement ->
            statement.setString(1, provider.trim().uppercase())
            statement.setString(2, symbol.trim().uppercase())
            statement.setLong(3, notBeforeMillis)
            statement.executeQuery().use { result -> if (result.next()) result.quote(symbol) else null }
        }
