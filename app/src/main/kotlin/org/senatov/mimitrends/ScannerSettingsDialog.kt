...
class ScannerSettingsDialog(owner: Window?, current: ScannerCriteria, private val service: ScannerSettingsService) {
    ...
    private val anomalyWindow = ComboBox(FXCollections.observableArrayList(AnomalyWindow.entries)).apply { value = current.anomalyWindow }
    ...
    private val scanInterval = TextField(current.scanIntervalSeconds.toString())
    private val resultLimit = TextField(current.resultLimit.toString())
    private val price = TextField(current.minPrice.toString())
    private val turnover = TextField(current.minSessionTurnover.toString())
    ...
    init {
        owner?.let(dialog::initOwner); dialog.title = "Scanner Settings"; dialog.headerText = "Local anomaly ranking"
        val grid = GridPane().apply { hgap = 12.0; vgap = 9.0; padding = Insets(12.0) }
        listOf("Anomaly period" to anomalyWindow, "Market" to marketRegion, "Scan interval, seconds" to scanInterval,
            "Maximum results" to resultLimit, "Minimum source price" to price,
            "Minimum session turnover" to turnover, "Baseline sessions" to sessions, "Display currency" to currency,
            "Symbols (no total limit)" to symbols).forEachIndexed { row, pair ->
            grid.add(Label(pair.first), 0, row); grid.add(pair.second, 1, row)
        }
        grid.add(Label("Yahoo Finance supplies OHLCV without a key. Results are ranked against each instrument's local baseline; all downloaded bars are retained in SQLite. The recommended interval is 180 seconds.").apply {
            isWrapText = true; maxWidth = 430.0
        }, 0, 9, 2, 1)
    ...
    private fun parse(): ScannerCriteria? = runCatching {
        ScannerCriteria(anomalyWindow = anomalyWindow.value, marketRegion = marketRegion.value,
            scanIntervalSeconds = scanInterval.text.toLong().coerceIn(60, 3_600),
            resultLimit = resultLimit.text.toInt().coerceIn(1, 50),
            minPrice = price.text.toDouble(), minSessionTurnover = turnover.text.toDouble(),
            baselineSessions = sessions.text.toInt().coerceIn(3, 20),
