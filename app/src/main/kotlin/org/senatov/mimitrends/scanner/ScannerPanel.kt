...
class ScannerPanel(
    ...
    init {
        ...
        val turnover = columnFactory.number(
            "Turnover",
            { convertPrice(it.symbol, it.sessionTurnover) },
            { ScannerValueFormat.money(it, currency) })
        ...
        listOf(freshness, symbol, signal, entryQuality, move, price, scoreColumn, outcome, priceAction, volume, age, turnover, updated)
            .zip(
                listOf(
                    "delay", "company", "pattern", "entry_quality", "move", "price", "anomaly", "outcome", "price_action",
        ...
        autoFitter = TableColumnAutoFitter(
            table, listOf(
                ...
                TableColumnAutoFitter.Spec(
                    move,
                    { ScannerValueFormat.percent(it.windowChangePercent) },
                    62.0,
                    105.0,
                    reserveWidth = 4.0
                ),
                TableColumnAutoFitter.Spec(
                    price,
                    { "${currency.symbol}%,.2f".format(convertPrice(it.symbol, it.price)) },
                    62.0,
                    110.0,
                    reserveWidth = 4.0
                ),
                ...
                TableColumnAutoFitter.Spec(
                    turnover,
                    { ScannerValueFormat.money(convertPrice(it.symbol, it.sessionTurnover), currency) },
                    88.0,
                    145.0,
                    reserveWidth = 8.0
                ),
                TableColumnAutoFitter.Spec(
                    updated,
                    { time.format(Instant.ofEpochMilli(it.updatedAtMillis)) },
                    88.0,
                    125.0,
                    reserveWidth = 8.0
                )
