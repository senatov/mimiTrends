...
internal object ResearchReportDialog {
    ...
    private fun content(reports: List<WalkForwardResearchReport>) = VBox(
        10.0,
        Label(
            "Every row is evaluated using outcomes from earlier trading days only. " +
                    "Returns include the configured 0.20% friction allowance."
        ).apply {
            isWrapText = true
    ...
    private fun emptyState(report: WalkForwardResearchReport) = VBox(
        8.0,
        Label("Not enough historical days yet"),
        Label(
            "Collected outcomes: ${report.outcomeSamples}. A family needs at least 20 earlier samples " +
                    "before a later day can be evaluated."
...
internal enum class ResearchReportChoice {
    CLOSE,
    EXPORT,
    BACKFILL
}