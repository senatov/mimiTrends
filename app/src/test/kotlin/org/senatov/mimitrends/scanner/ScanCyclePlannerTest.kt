...
class ScanCyclePlannerTest {
    ...
    @Test
    fun `keeps recent candidates ahead of the ordinary rotation`() {
        val planner = ScanCyclePlanner()
    ...
    @Test
    fun `bounds large cycles and rotates through the full universe`() {
    ...
    @Test
    fun `rotates both regions without starving either one`() {
        val planner = ScanCyclePlanner(maximumSymbolsPerCycle = 4)
