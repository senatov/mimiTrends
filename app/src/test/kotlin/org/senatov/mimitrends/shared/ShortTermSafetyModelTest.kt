...
class ShortTermSafetyModelTest {
    @Test
    fun `negative acceleration below vwap blocks an Apple-like reversal`() {
        val now = epoch(11, 0)
    ...
    @Test
    fun `orderly liquid regular-session setup can qualify`() {
        val now = epoch(11, 0)
        val assessment = ShortTermSafetyModel.assess(
    ...
    @Test
    fun `premarket data cannot produce an actionable high score`() {
        val now = epoch(8, 45)
        val assessment = ShortTermSafetyModel.assess(
    ...
    private fun features(now: Long, one: Double, three: Double, five: Double, vwap: Double, efficiency: Double) =
        ResearchFeatures(
            now, 100.0, one, three, five, five, five, five, 0.3, 0.15, vwap,
