...
class UiStateService(private val path: Path = Path.of(System.getProperty("user.home"), ".mimi", "trends", "ui-state.properties")) {
    ...
    private fun attachTracking(stage: Stage) {
        ...
        val update = { _: Any? ->
            if (!stage.isMaximized && !stage.isFullScreen && stage.width > 0 && stage.height > 0) normalBounds =
                Rectangle2D(stage.x, stage.y, stage.width, stage.height)
        }
        stage.xProperty().addListener { _, _, value -> update(value) }; stage.yProperty().addListener { _, _, value -> update(value) }
        stage.widthProperty().addListener { _, _, value -> update(value) }; stage.heightProperty()
            .addListener { _, _, value -> update(value) }
    }
