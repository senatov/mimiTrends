...
class ScannerSettingsDialog(
    ...
    private val currency =
        ComboBox(FXCollections.observableArrayList(DisplayCurrency.entries)).apply { value = current.displayCurrency }
    ...
    init {
        ...
        val scanner = VBox(
            ...
            section(
                ...
                settingRow(
                    ...
                    "The scanner relaxes thresholds gradually when necessary to fill approximately this many rows.",
                    minimumResults
                ),
                settingRow(
                    "Maximum results",
                    "Hard display limit. Candidates below the limit are ranked by quality; extra weak rows are not appended.",
                    resultLimit
                ),
            ...
            section(
                ...
                settingRow(
                    ...
                    "Minimum percentage movement required before statistical anomalies are considered.",
                    absoluteMove
                ),
                ...
                settingRow(
                    "Minimum candle body",
                    "Body divided by candle range. Higher values reject wick-heavy and indecisive candles.",
                    bodyRatio
                )
            ),
            section(
                ...
                settingRow(
                    "Trend efficiency",
                    "Net progress divided by total travelled path. Higher values require a smoother rise.",
                    trendEfficiency
                )
            ),
            section(
                "Confirmation and data quality",
            ...
            section(
                ...
                settingRow(
                    "Finnhub live feed",
                    "Optional. A new key is stored locally; blank keeps the existing configuration.",
                    finnhubApiKey
                )
            ),
            section(
        ...
        val providers = VBox(
            ...
            section(
                "Tradegate",
                settingRow(
                    "Public quote collector",
                    "Resolve company names to Tradegate instruments and collect EUR quotes during its weekday trading session.",
                    tradegateEnabled
                ),
                settingRow(
                    "Request interval",
                    "Milliseconds between sequential instruments. A small timing jitter and automatic backoff are applied.",
                    tradegateInterval
                )
            ),
            section(
                ...
                settingRow(
                    "Public quote collector",
                    "Resolve ISIN and MIC through Euronext search and collect delayed market-information quotes.",
                    euronextEnabled
                ),
                settingRow(
                    "Request interval",
    ...
    private fun parse(): ScannerSettingsResult? = runCatching {
        val criteria = ScannerCriteria(
            anomalyWindow = AnomalyWindow.HOUR, marketRegion = marketRegion.value,
