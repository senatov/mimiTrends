...
class ProviderInstrumentSelectorTest {
    @Test
    fun `rejects an unrelated cached company and selects the listing market instrument`() {
    ...
    @Test
    fun `accepts equivalent names with different legal suffixes`() {
        assertEquals(
    ...
    @Test
    fun `rejects TXNM Energy for Texas Instruments ticker`() {
        assertFalse(
            ProviderInstrumentSelector.matchesCompany(
    ...
    @Test
    fun `prefers isin identity over an ambiguous ticker and company name`() {
    ...
    @Test
    fun `does not fall back to a conflicting ticker candidate when isin is known`() {
        val wrong = instrument("EURONEXT", "US8261975010", "Siemens AG")
    ...
    @Test
    fun `rejects a cached provider instrument with a conflicting isin`() {
        val wrong = instrument("EURONEXT", "US8261975010", "Siemens AG")
