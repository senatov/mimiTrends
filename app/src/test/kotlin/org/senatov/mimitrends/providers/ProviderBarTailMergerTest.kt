...
class ProviderBarTailMergerTest {
    @Test
    fun `appends a sufficiently dense Tradegate tail after Yahoo history`() {
        ...
        assertEquals(
            listOf(60L, 120L, 180L, 240L, 300L, 360L, 420L),
    ...
    @Test
    fun `accepts the normal five minute provider rotation as an analytic tail`() {
        val yahoo = listOf(bar(60, 100.0))
        val providers = (1L..6L).map { index -> provider("TRADEGATE", 60L + index * 300L, 100.0 + index) }

        val merged = ProviderBarTailMerger.merge(yahoo, providers, MarketDataSource.YAHOO, 1_900)

        assertEquals(
            providers.takeLast(3).map { it.bar.minuteEpochSeconds },
            merged.analysisBars.drop(1).map(MinuteBar::minuteEpochSeconds)
        )
        assertEquals(true, merged.analysisTracksLatestQuote())
    }

    @Test
    fun `uses a lone live quote to extend current history for analysis`() {
        val yahoo = listOf(bar(60, 100.0), bar(120, 101.0))
    ...
    @Test
    fun `live provider replaces overlapping Yahoo candle`() {
        val yahoo = listOf(bar(60, 100.0), bar(120, 101.0))
    ...
    @Test
    fun `uses the freshest provider instead of a fixed provider priority`() {
        val yahoo = listOf(bar(60, 100.0))
        val providers = listOf(
    ...
    @Test
    fun `keeps the denser analytic tail while exposing the freshest quote`() {
        val yahoo = listOf(bar(60, 100.0))
        ...
        assertEquals(
            listOf(60L, 120L, 180L, 240L, 300L, 360L),
            merged.analysisBars.map(MinuteBar::minuteEpochSeconds)
        )
    ...
    @Test
    fun `does not analyze a dense tail that ended long before the freshest quote`() {
        val yahoo = listOf(bar(60, 100.0))
        val providers = (2L..6L).map { provider("TRADEGATE", it * 60L, 99.0 + it) } +
                provider("LANG_SCHWARZ", 900, 106.0)

        val merged = ProviderBarTailMerger.merge(yahoo, providers, MarketDataSource.YAHOO, 960)
    ...
    @Test
    fun `ignores provider observations older than fifteen minutes`() {
        val yahoo = listOf(bar(60, 100.0))
    ...
    @Test
    fun `older provider tail never rolls back newer primary data`() {
